...
export async function processTaskIpc(
  ...
  switch (data.type) {
    ...
    case 'context_update': {
      ...
      // Files at these paths still must end .md, must not contain .., and the
      // resolved path must remain inside no5-context. Basename is checked against
      // BLOCKED_FILES regardless of subdirectory.
      ...
      // Reject path traversal and require .md
      ...
      // Enforce: either flat filename, or exactly one allowed subdirectory + filename
      const segments = file.split('/');
      if (segments.length > 2) {
        logger.warn(
          { file, sourceGroup },
          'context_update blocked — nested subdirectories not allowed',
        );
        break;
      }
      if (segments.length === 2 && !ALLOWED_SUBDIRS.has(segments[0])) {
        logger.warn(
          { file, subdir: segments[0], sourceGroup },
          'context_update blocked — subdirectory not in allowlist',
        );
        break;
      }
      ...
      if (!basename || basename.startsWith('.')) {
        logger.warn(
          { file, sourceGroup },
          'context_update blocked — invalid basename',
        );
        break;
      }
